<x-guest-layout>
    ...
    <div class="bg-white">
        <div>
          <!-- Mobile filter dialog  Off-canvas menu for mobile, show/hide based on off-canvas menu state.-->
              ...
              <!-- Breadcrumb -->
                <div class="text-sm breadcrumbs">
                    <ul>
                        ...
                        <li><a>Category {{ $category->name }}</a></li> 
                    </ul>
                </div>
              <h1 class="text-4xl font-extrabold tracking-tight text-gray-900">All {{ $category->name }}</h1>
              <p class="mt-4 text-base text-gray-500">Checkout out the latest release of products, new and improved with four openings!</p>
            </div>
                    ...
                    <div class="max-w-7xl mx-auto px-4 overflow-hidden sm:px-6 lg:px-8">
                      <div class="grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-3 lg:gap-x-8">
                        ...
                        @foreach ($products as $prod)
                            <a href="{{ url('view-category/'. $category->slug.'/'.$prod->slug) }}" class="group text-sm">
                                <div class="w-full aspect-w-1 aspect-h-1 rounded-lg overflow-hidden bg-gray-100 group-hover:opacity-75">
                                    <img src="{{ asset('assets/uploads/product/' . $prod->image) }}" alt="product-img">
                                </div>
                                <h3 class="mt-4 font-medium text-gray-900">{{ $prod->name }}</h3>
                                <p class="text-gray-500 italic">{{ $prod->small_description }}</p>
                                <p class="mt-2 font-medium text-gray-900">MYR {{ $prod->selling_price }}</p>
                            </a>
                        @endforeach
